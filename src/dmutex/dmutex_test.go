...
func newKeysAPI(hosts []string) client.KeysAPI {
	cfg := client.Config{
		Endpoints:               hosts,
...
func TestMutex(t *testing.T) {
	...
	m := NewMutex(lockKey, 60, machines)
	if m == nil {
...
func TestLockConcurrently(t *testing.T) {
	...
	machines := []string{"http://127.0.0.1:2379"}
	kapi := newKeysAPI(machines)
	m1 := NewMutex(lockKey, 60, machines)
	m2 := NewMutex(lockKey, 60, machines)
	m3 := NewMutex(lockKey, 60, machines)
...
func TestLockTimeout(t *testing.T) {
	slice := make([]int, 0, 2)
	m1 := NewMutex("key", 2, []string{"http://127.0.0.1:2379"})
	m2 := NewMutex("key", 2, []string{"http://127.0.0.1:2379"})
	m1.Lock()
	ch := make(chan bool)
	go func() {
		m2.Lock()
